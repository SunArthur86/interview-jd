...
- 事务证据链
feynman:
  essence: 线上出现频繁死锁告警。你会从哪些 MySQL 信息中拿到证据，定位到底是哪两类 SQL、哪两个事务发生了死锁的核心不是背概念，而是先识别业务目标、流量形态、失败模式和一致性要求，再用可观测、可回滚、可扩展的工程手段落地。
...
> 我会先确认业务目标和 SLA。对于“线上出现频繁死锁告警。你会从哪些 MySQL 信息中拿到证据，定位到底是哪两类 SQL、哪两个事务发生了死锁”，核心是 MySQL 与 死锁 的平衡。我的方案是：主链路保持简单可靠，关键状态落 MySQL 并用唯一键/版本号保证幂等；热点读用缓存但设置失效和回源保护；非核心动作走 MQ 异步，消费端做幂等和补偿；入口到下游都配置超时、限流、熔断和降级。最后通过压测、灰度、监控和回滚预案验证。
